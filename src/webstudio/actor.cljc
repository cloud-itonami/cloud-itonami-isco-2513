(ns webstudio.actor
  "WebMultimediaActor — the ISCO-08 2513 community webstudio actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one webstudio operation request
  ...
  The unconditional invariant: the WebMultimediaAdvisor can never
  directly commit a record the WebMultimediaGovernor refuses —
...
(defn build-graph
  "Build a compiled WebMultimediaActor graph. `store` implements
  `webstudio.store/Store`. `advisor` implements
  `webstudio.advisor/Advisor` (defaults to `mock-advisor`).
